    ...
    function execute() {
      ...
      if ( isset($HTTP_GET_VARS['products_id']) && defined('MODULE_SOCIAL_BOOKMARKS_INSTALLED') && tep_not_null(MODULE_SOCIAL_BOOKMARKS_INSTALLED) ) {
        ...
        if ( !empty($social_bookmarks) ) {
          $data = '<div class="ui-widget infoBoxContainer" style="margin-top: 25px;">' .
                  '  <div class="ui-widget-content infoBoxContents" style="text-align: center;">' . MODULE_BOXES_PRODUCT_SOCIAL_BOOKMARKS_BOX_TITLE . ': ' . implode(' ', $social_bookmarks) . '</div>' .
                  '</div>';
